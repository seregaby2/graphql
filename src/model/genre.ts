...
export const getGenres = async (_:null, data:IGenre, context: IContext) => {
  const res: IGenre[] = await methodGet(URL.URL_GENRE, data, context.token);
  return res;
};

export const getGenre = async (_:null, data:IGenre, context: IContext) => {
  const res: IGenre = await methodGet(URL.URL_GENRE, data, context.token);
  return res;
};
